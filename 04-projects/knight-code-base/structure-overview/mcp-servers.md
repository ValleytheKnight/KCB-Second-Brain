...
Knight Code registers 6 MCP servers in its own .mcp.json, each exposing tools the session calls directly (the mcp__<server>__<tool> naming Claude Code uses). Structural graph servers, like knightbrain and lorebrain, reindex on every query, so they never go stale between sessions.
## Registered servers
...
| `spec-workflow` | `npx -y @pimzino/spec-workflow-mcp@latest C:\Users\Chris Brown\Documents\Knight Code` |
